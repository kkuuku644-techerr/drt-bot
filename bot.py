...
async def play_slots(message: Message):
  args = message.text.split()
  if len(args) < 2:
    return await message.answer("❌ Формат: `/slots <ставка>`", parse_mode="Markdown")

  clean_bet = args[1].replace("<", "").replace(">", "")
  if not clean_bet.isdigit() or int(clean_bet) < 5:
    return await message.answer("❌ Ставка от 5 монет: `/slots <ставка>`", parse_mode="Markdown")
...
async def play_dice(message: Message):
  args = message.text.split()
  if len(args) < 2:
    return await message.answer("❌ Формат: `/dice <ставка>`", parse_mode="Markdown")

  clean_bet = args[1].replace("<", "").replace(">", "")
  if not clean_bet.isdigit() or int(clean_bet) < 5:
    return await message.answer("❌ Ставка от 5 монет: `/dice <ставка>`", parse_mode="Markdown")
...
async def play_mines(message: Message):
  args = message.text.split()
  if len(args) < 2:
    return await message.answer("❌ Формат: `/mines <ставка>`", parse_mode="Markdown")

  clean_bet = args[1].replace("<", "").replace(">", "")
  if not clean_bet.isdigit() or int(clean_bet) < 5:
    return await message.answer("❌ Ставка от 5 монет: `/mines <ставка>`", parse_mode="Markdown")
